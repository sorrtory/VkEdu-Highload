...
- Запись в ScyllaDB идет в любой ДЦ с `LOCAL_QUORUM`. Используется `NetworkTopologyStrategy`, RF=3 в каждом ДЦ. Между ДЦ данные реплицируются асинхронно, модель консистентности - eventual consistency.
- Для мессенджера операции конкретного диалога маршрутизируются в его домашний ДЦ. Внутри домашнего ДЦ сохраняются порядок и read-after-write, а меж-ДЦ копия истории сходится eventual consistency.
...
ScyllaDB хранит высоконагруженные сущности, которые читаются по заранее заданным ключам: посты, комментарии, реакции, ленты и сообщения. Запись идет в локальный ДЦ с `LOCAL_QUORUM`; используется `NetworkTopologyStrategy` с RF=3 в каждом ДЦ, между ДЦ данные сходятся асинхронно.
...
| ScyllaDB   | `NetworkTopologyStrategy`, RF=3 в каждом ДЦ. Запись идет в любой ДЦ с `LOCAL_QUORUM`. Между ДЦ данные реплицируются eventual consistency       |

...
- MSK - primary, SPB и EKB - secondary
...
- ScyllaDB разделена на два режима. Social keyspace использует `NetworkTopologyStrategy`: RF=1 в каждом ДЦ, суммарно RF=3; строгие операции идут с `QUORUM`.
- Для мессенджера операции конкретного диалога маршрутизируются в его `home_dc`. Используется family keyspace: `messenger_msk`, `messenger_spb`, `messenger_ekb`. В домашнем ДЦ история пишется и читается с RF=3 и `LOCAL_QUORUM`; меж-ДЦ копии истории сходятся асинхронно.
...
| `auth-service`           | Регистрация, логин, refresh token, logout           | `users`, `user_sessions`, Redis session cache                                  |
...
flowchart TB
    ...
    Services -->|cache / session lookup / rate limit| Redis
...
ScyllaDB хранит высоконагруженные сущности, которые читаются по заранее заданным ключам: посты, комментарии, реакции, ленты и сообщения.

Для social keyspace используется единый multi-DC режим: `NetworkTopologyStrategy`, RF=1 в каждом ДЦ, суммарно RF=3. Запись и строгое чтение идут с `QUORUM`; кеши и read-model могут читаться локально или с более слабой консистентностью, если допускают устаревание.

Для messenger keyspace family используется маршрутизация по `conversations.home_dc`: `messages`, `conversation_participants`, `conversation_participants_by_user` и `user_conversations` пишутся в `messenger_msk`, `messenger_spb` или `messenger_ekb` с RF=3 и `LOCAL_QUORUM` внутри домашнего ДЦ диалога. Асинхронные меж-ДЦ копии нужны для failover и восстановления read-model, но не находятся в синхронном пути отправки сообщения.
...
Redis используется как локальный кеш в каждом ДЦ: session lookup, первые страницы ленты, inbox и счетчики. При потере кеша данные восстанавливаются из PostgreSQL или ScyllaDB.
...
flowchart TB
    ...
    Auth -->|read/write session cache| UserSessions
...
| ScyllaDB   | Social keyspace: RF=1 на ДЦ, суммарно RF=3, строгие операции с `QUORUM`. Messenger keyspace family: `home_dc` диалога, RF=3 внутри домашнего ДЦ, `LOCAL_QUORUM`, меж-ДЦ копии асинхронные |

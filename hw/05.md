...
| **`files`**                     | Metadata файлов: изображения, видео, аватары, вложения | id(8) + file_type(16) + mime_type(32) + storage_bucket/key(160) + cdn_url/preview_url(256) + size/checksum/status/dates(96) ≈ 560 Б                            | 2 млрд           | ≈ 1,12 ТБ      | 216.93                      | 15 313                      |
| **`file_payloads`**             | Бинарное содержимое файлов в объектном хранилище       | средний объект ≈ 200 КБ                                                                                                                                        | 2 млрд           | ≈ 400 ПБ       | 216.93                      | CDN / s3 origin read        |
...
- `files`, `file_payloads` и `post_attachments` привязаны к публикации поста. Для `post_attachments` учтено несколько вложений на один пост.
